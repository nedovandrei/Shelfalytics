    ...
    public class MailService : IMailService
    {
        ...
        public async Task SendOOSEmail(ProductOOSDTO product, int equipmentId)
        {
            ...
            using (var client = new SmtpClient("mail.shelf.work"))
            {
                await Task.Run(delegate
                {
                    ...
                    mail.From = new MailAddress("notify@shelf.work");
                    mail.Subject = "Out Of Stock Notification";
                    mail.IsBodyHtml = true;
                    //mail.Body = "<html><body><font color=\"red\">OOS NOTIFICATION TEST</font></body></html>";
                    mail.Body = File.ReadAllText(HostingEnvironment.MapPath("~/Content/MessageTemplates/OOSNotification.html"));
        ...
        private void ExceptionCatcher(Exception ex)
        {
            using (var client = new SmtpClient("mail.shelf.work"))
            {
                ...
                var mail = new MailMessage();
                mail.From = new MailAddress("notify@shelf.work");
                mail.Subject = "Mailing Exception";
                mail.Body = "Sending Email Failed" + "\n\n" + ex.Message + "\n\n" + ex.Source + "\n\n" + ex.StackTrace;
